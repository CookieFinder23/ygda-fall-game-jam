...
@onready var hunter_animated_sprite: AnimatedSprite2D = $HunterAnimatedSprite
@onready var knight_animated_sprite: AnimatedSprite2D = $KnightAnimatedSprite
...
func _ready() -> void:
	character_data = {
	Character.HUNTER: [MovementSpeed.NORMAL, AttackCooldownLength.NORMAL, MovementAbilityCooldownLength.NORMAL, hunter_animated_sprite],
	Character.KNIGHT: [MovementSpeed.NORMAL, AttackCooldownLength.NORMAL, MovementAbilityCooldownLength.NORMAL, knight_animated_sprite]
	}
	current_animated_sprite = hunter_animated_sprite
...
func set_animated_sprite(new_animated_sprite: Character) -> void:
	for character in character_data:
		character_data[character][3].visible = character == new_animated_sprite
	current_animated_sprite = character_data[new_animated_sprite][3]
	
func set_character(character: Character) -> void:
	current_character = character
	attack_cooldown.stop()
	movement_ability_cooldown.stop()
	var old_animation = current_animated_sprite.animation
	set_movement_speed(character)
	set_attack_cooldown(character)
	set_movement_ability_cooldown(character)
	set_animated_sprite(character)
	play_animation(old_animation)
	
	
func play_animation(animation: String):
	current_animated_sprite.play(animation)

func do_movement_ability(character: Character):
